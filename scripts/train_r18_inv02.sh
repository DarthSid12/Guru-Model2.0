#!/usr/bin/env bash
# r18_inv02 -- r16_rfw's recipe, byte-identical apart from --invert-p 0.02.
#
...
# collapsing congruence, while upright still being 98% of the diet preserves
...
#
# WHY 2%. The 5% and 20% arms both ABOLISHED the classification inversion
# effect outright (faces valid/test-inverted: 0% 90/62, 5% 86/86, 20% 87/86;
# overall gap 17.95 -> 2.18 -> 1.54). 20% adds nothing over 5%, so the effect
# saturates well below 5% and the informative exposures are lower. This arm
# samples that region. If 2% also flattens it, the next point is ~0.5-1%.
#
