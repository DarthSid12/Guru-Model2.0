"""
simulate_kanwisher2023.py

Replicate the inversion experiment of Dobs, Yuan, Martinez & Kanwisher (2023),
"Behavioral signatures of face perception emerge in deep neural networks
optimized for face recognition" (PNAS 120(32), e2220642120), for a single
category (faces / houses_zubud / objects) using the trained log-polar network.

How this differs from simulate_yin1969.py
-----------------------------------------
Yin (1969) is a *recognition memory* paradigm: study 40 items, then decide which
of two test items feels familiar (KDE familiarity over a noisy memory bank).
Dobs et al. is a *perceptual discrimination* paradigm with no memory phase at
all -- their Experiments 1 and 5 use a three-image "target-matching" task:

    target T is shown with two candidates; one (A) is a DIFFERENT photo of the
    same identity, the other (B) is a different identity. Pick the candidate
    whose representation is closest to the target.

In the CNN version they read the penultimate fully connected layer, take the
correlation distance (1 - Pearson r) between the target and each candidate, and
model the choice as argmin distance (their Fig. 1B, right). Experiment 5 then
repeats the identical task with every image presented upside-down, and reports
the drop in matching accuracy -- the face inversion effect (Face-ID CNN: 86.9%
upright vs 66.4% inverted; their Fig. 3B).

So, relative to the Yin simulation:
  - no study phase, no memory bank, no KDE -- accuracy is read straight off a
    similarity judgement between three images
  - two conditions (Upright, Inverted), not four: orientation is a property of
    the whole trial, never crossed between study and test
  - the read-out mirrors Yin's noisy code exactly: stochastic sampling ON
    (bernoulli draw from the 256-d sigmoid probs -> binary h), then the same
    bit-flip retrieval noise (`apply_binomial_noise`, rate p), averaged over an
    image's fixations. The noise level p is calibrated the same way as Yin: swept
    until the Upright condition's matching accuracy matches a human target
    (`--calib-target`, default 0.875 = the Dobs et al. Exp. 5 human upright
    accuracy) -- the Kanwisher analogue of Yin's "upright-upright" calibration,
    since this paradigm has no separate study phase to cross.

Their headline conclusion is that this inversion effect is not face-specific but
follows from optimizing for fine-grained discrimination *within* a category on
upright images (a car-trained net shows a car inversion effect; a net trained on
inverted faces shows the reverse effect). Running the same task over our faces /
houses / objects categories is a direct test of that claim in a log-polar,
fixation-driven network.

Design
------
  - identities : `--num-identities` classes of the category (default 40, matching
                 their Exp. 1 stimulus set), each contributing up to
                 `--images-per-identity` held-out photos (default 5, also theirs)
  - trials     : every ordered same-identity pair (T, A) is tested against every
                 image B of every other sampled identity, so each trial is a
                 genuine 2AFC and accuracy is the fraction of (T, A, B) triplets
                 where dist(T, A) < dist(T, B). Chance is 50%.
  - conditions : upright  -> OnTheFlyTransform("valid")  (identity rotation)
                 inverted -> OnTheFlyTransform("test")   (180-degree rotation)
                 applied to *all three* images of a trial.

Images come from the packed fixation store (preprocess_fixations.py). The task
needs >= 2 held-out photos per identity, so `--splits` controls where they come
from: faces/objects have many photos per class in 'valid' alone, while
houses_zubud is packed one photo per house per split (view 4 in valid, view 5 in
test), so it needs `--splits valid test` to get a genuine cross-view pair. Both
of those views are held out of training (train is views 1-3).

Example (point --run-dir at a training run to auto-load config / checkpoint):
    python simulate_kanwisher2023.py --category faces \
        --run-dir runs/faces_objects_houses_zubud_lp_16fix_lr0.001_resnet18_r5b_houses_zubud_30ep

    python simulate_kanwisher2023.py --category houses_zubud --splits valid test \
        --run-dir runs/<run>
"""
...
def load_fixations(sp, img_idx, num_fixations, crop_size=180):
    """uint8 tensor (num_fixations, 3, crop_size, crop_size) for one packed photo."""
    img = sp.images[img_idx]
    return torch.stack([
        _crop_at(img, sp.coords[img_idx, j, 0], sp.coords[img_idx, j, 1], crop_size)
        for j in range(num_fixations)], dim=0)
...
def encode_image(model, transform, crops, device, p_noise):
    """Photo -> one 256-d noisy binary code, averaged over its fixations.

    Mirrors simulate_yin1969.py's encode(): stochastic sampling of h (a
    bernoulli draw from the sigmoid probs), then bit-flip retrieval noise at
    rate p_noise, before averaging over the photo's fixations. The analogue of
    Dobs et al. reading the penultimate fully connected layer, but with the
    same noisy read-out as the Yin simulation instead of a clean deterministic
    one.
    """
    x = transform(crops.to(device))
    model.stochastic = True
    _, h, _ = model(x, return_rep=True)
    h_noisy = apply_binomial_noise(h.cpu(), p_noise)
    ...
    with torch.no_grad():
        for cls, imgs in identities.items():
            for split, sp, img_idx in imgs:
                crops = load_fixations(sp, img_idx, num_fixations)
                feats[(cls, split, img_idx)] = encode_image(model, transform, crops, device, p_noise)
