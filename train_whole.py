...
updated training script which has the option of training on whole images rather than fixations.

if you want whole training, here's an example

python3 train_whole.py --categories faces --variant lp --data-mode whole 
"""
...
from datasets import make_datasets, make_packed_datasets, make_whole_datasets
from salience_trans import OnTheFlyTransform, OnTheFlyWholeTransform
...
def evaluate(model, loader, device, num_classes, transform=None, amp=False, channels_last=False):
    ...
    for inputs, labels in loader:
        ...
        # different based on whether we train with whole images or not
        if inputs.dim() == 5:
...
def main():
    ...
    fix_tag = "whole" if args.data_mode == "whole" else f"{args.num_fixations}fix"
    out_dir = args.output_dir or (
        f"runs/{'_'.join(args.categories)}_{args.variant}_"
        f"{fix_tag}_lr{args.lr}_{args.backbone}"
    ...
    elif args.data_mode == "whole":
        datasets, label_map = make_whole_datasets(
            categories=args.categories,
            data_root=args.whole_root,
            max_images_per_class=max_images_per_class,
        )
    ...
    valid_batch_size = args.batch_size if whole_mode else max(1, args.batch_size // args.num_fixations)
    ...
    # ----------------------------- Model ----------------------------
    model = Model(size=224 if whole_mode else 180, num_classes=num_classes, pretrained=args.pretrained,
